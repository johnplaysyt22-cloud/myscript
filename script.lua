if getgenv().ifanzio_Running then
...
local function PostToWebhook()
    ...
    local payload = {
        ["embeds"] = {{
            ...
            ["footer"] = { ["text"] = string.format("ifanzio • Session: %s • %s", GetSessionTime(), os.date("%x %X")) },
...
local function SendSafetyWebhook(targetPlayer, reason)
    ...
    local payload = {
        ["embeds"] = {{
            ...
            ["footer"] = { ["text"] = "ifanzio • " .. os.date("%x %X") }
        }}
    }
...
local Window = Library:CreateWindow({
	Title = "ifanzio",
	Footer = "" .. assetName .. " | IFANZIO | SKIDDED",
	NotifySide = "Right",
...
task.spawn(function()
    while task.wait(5) do
        if not getgenv().ifanzio_Running then break end
...
task.spawn(function()
    while getgenv().ifanzio_Running do
...
ThemeManager:SetFolder("ifanzio")
SaveManager:SetFolder("ifanzio/SailorPiece")
